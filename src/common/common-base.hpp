...
namespace cyclic
{
    ...
    */
    ...
    public:
        ...
        /**
         * Return the fieldset to which the record is attached.
         * @return The attached fieldset, nullptr if not attached.
         */
        virtual const cyclic::fieldset* fieldset()const =0;
    ...
    public:
        ...
        /**
         * Attach the record to a fieldset.
         * @param fieldset Field to which attach the record.
         * @return this
         */
        virtual record& attach(const cyclic::fieldset* fieldset) =0;
    ...
    {
    private:
        const cyclic::fieldset* _fieldset = nullptr;
    ...
    public:
        ...
        /**
         * Create a raw record, eventually attached to a fieldset.
         * @param fieldset Fieldset to which attach the record. Can be nullptr.
         * @param record Record index. Invalid by default.
         * @param time Time of the record.
         */
        raw_record(const cyclic::fieldset* fieldset, index_t record = invalid_index(), time_t time = 0);

        raw_record(index_t index, time_t time, const std::vector<value_t>& values);
        raw_record(index_t index, time_t time, std::vector<value_t>&& values);
        raw_record(index_t index, time_t time, const std::map<cyclic::field_index_t, cyclic::value_t>& values);
        raw_record(std::optional<index_t> index, std::optional<time_t> time, const std::map<cyclic::field_index_t, cyclic::value_t>& values):
            raw_record(index.value_or(0), time.value_or(0), values) {}
        ...
        const cyclic::fieldset* fieldset()const override;
        index_t index()const override;
        time_t time()const override;
        bool ok()const override;
        record& attach(const cyclic::fieldset* fieldset) override;
     ...
     */
    /* abstract */ class recordset : public fieldset
    {
    public:
     ...
     */
    /* abstract */ class table : public fieldset
    {
    public:
        ...
        /**
         * Return a recordset representing the current whole table content.
         * @return A recordset filled with all records of the table.
         */
        virtual std::unique_ptr<recordset> get_recordset() const =0;

        /**
         * Return a recordset representing a range of the table content.
         * @param first First record index to include.
         * @param last Last record index to include.
         * @param count Maximum number of records to include.
         * @return A recordset filled with records of the table in the specified range.
         */
        virtual std::unique_ptr<recordset> get_recordset(record_index_t first, record_index_t last = record::absolute_max_index(), record_index_t count = std::numeric_limits<record_index_t>::max()) const =0;
