...
raw_record::raw_record(const cyclic::fieldset* fieldset, record_index_t record, record_time_t time) :
_fieldset(fieldset),
_index(record),
_time(time),
_values(fieldset->field_count())
{
}

raw_record::raw_record(const raw_record& rec):
_fieldset(rec._fieldset),
_index(rec._index),
_time(rec._time),
_values(rec._values)
{
...
raw_record::raw_record(index_t index, time_t time, const std::vector<value_t>& values):
_index(index),
_time(time),
_values(values)
{
...
raw_record::raw_record(index_t index, time_t time, std::vector<value_t>&& values):
_index(index),
_time(time),
...
raw_record::raw_record(index_t index, time_t time, const std::map<cyclic::field_index_t, cyclic::value_t>& values):
_index(index),
_time(time)
{
    field_index_t max = 0;
    ...
    _values.resize(max+1);
    for(const auto& entry : values) {
        _values[entry.first] = entry.second;
    }
}
...
const cyclic::fieldset* raw_record::fieldset()const
{
    return _fieldset;
}
...
record& raw_record::attach(const cyclic::fieldset* fieldset)
{
    _fieldset = fieldset;
...
{
    if(_fieldset==nullptr)
    {
        throw cyclic::detached_record{};
    }
    else
    {
        return _fieldset->field(field);
    }
}
...
const cyclic::field& raw_record::field(const std::string& field_name)const
{
    ...
    {
        return _fieldset->field(field_name);
    }
}
...
mutable_record& raw_record::set(field_index_t field, value_t value)
{
    ...
    _values[field] = _fieldset ? cast_value_to(value, this->field(field).type()) : value;
    return *this;
}
...
mutable_record& raw_record::set(const std::string& field_name, value_t value)
{
    ...
    else
    {
        ...
        _values[index] = cast_value_to(value, type);
    }
    return *this;
}
