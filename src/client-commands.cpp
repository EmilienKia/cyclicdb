...
bool timeseries_select::execute(const execution_context& ctx, cyclic::client::connection &cnx) {
    ...
    // TODO select by ranged-recordset
    // TODO Print headers
    // TODO factorize code with store commands
    for(cyclic::record_index_t idx = first; idx <= last; ++idx) {
        auto rec = ts->get_record(idx);
        if(rec) {
            std::cout << idx;
            for(size_t n=0; n<colnames.size(); ++n)
            {
                std::cout << "\t" << val_to_str((*rec)[colnames[n]]);
            }
            std::cout << std::endl;
        }
    }
