...
//        end = request.param("time_end").as<cyclic::record_time_t>();
    ...
    process<timeseries_data, cyclic::proto::GetTimeseriesDataResponse>(request, response, [this, dbname, tsname, start, end](){
        auto res = _server->get_timeseries_data(dbname, tsname, start, end);
        return res;
    });
}
