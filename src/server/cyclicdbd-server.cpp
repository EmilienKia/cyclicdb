...
/*
 * src/server/cyclicdbd-server.cpp
...
expected<timeseries_data> cyclic_server::get_timeseries_data(const std::string& dbname, const std::string& tsname, std::optional<cyclic::record_index_t> start, std::optional<cyclic::record_index_t> end, std::optional<cyclic::record_index_t> count) {
    auto db = _service->get_database(dbname);
    if(!db) {
