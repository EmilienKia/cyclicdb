...
namespace commands
{
    ...
    std::shared_ptr<cyclic::store::impl::file_table_impl> create::execute(const std::string& filename)
    {
        ...
        std::shared_ptr<cyclic::table> table = cyclic::store::file::create(filename, cyclic::store::file::COMPACT, fields, capacity);
...
bool command_executor::execute(commands::command* command)
{
    if(commands::query* query = dynamic_cast<commands::query*>(command))
    {
        ...
        if(!table)
        {
            std::shared_ptr<cyclic::table> tbl = cyclic::store::file::open(filename);
            if(!tbl)
            {
