...
/*
 * src/store/libstore-base-impl.cpp
...
field_index_t base_recordset_impl::field_count() const
{
    return _field_count;
}

const field_impl& base_recordset_impl::field(field_index_t field)const
{
...
const cyclic::field& base_recordset_impl::field(const std::string& field_name)const
{
...
record_index_t base_recordset_impl::min_index()const
{
    return record_count()>0 ? _first_index : record::invalid_index();
}

record_index_t base_recordset_impl::max_index()const
{
    return record_count()>0 ? _first_index + record_count() - 1 : record::invalid_index();
}



//
// base_recordset_impl::iterator
//
base_recordset_impl::iterator::iterator(const base_recordset_impl* recset, record_index_t index):
_recordset(recset),
_index(index)
{
}

bool base_recordset_impl::iterator::ok()const
{
    if(!_recordset)
    {
        return false;
    }
    if(_recordset->record_count()==0)
    {
        return false;
    }
    return _index <= _recordset->max_index();
}

void base_recordset_impl::iterator::increment()
{
    if(_recordset && _index!=record::invalid_index())
    {
        _index++;
        _rec.reset();
    }
...
bool base_recordset_impl::iterator::equals(const cyclic::recordset::const_iterator_interface* other)const
{
    const base_recordset_impl::iterator* it = dynamic_cast<const base_recordset_impl::iterator*>(other);
    if(it==nullptr)
    {
        return false;
    }
    if(_recordset!=it->_recordset)
    {
        return false;
    }
    if(_index<=_recordset->max_index())
    {
        return _index == it->_index;
    }
    else
    {
        return it->_index > it->_recordset->max_index();
    }
}

const record& base_recordset_impl::iterator::dereference()
{
    if(!_rec)
    {
        if(_recordset!=nullptr)
        {
            _rec = _recordset->get_record(_index);
        }
    }
    return *_rec;
}
...
const_recordset_iterator base_recordset_impl::begin()const
{
    return const_recordset_iterator{std::make_shared<iterator>(this, min_index())};
}

const_recordset_iterator base_recordset_impl::end()const
{
...
std::unique_ptr<recordset> base_table_impl::get_recordset() const
{
    return get_recordset(min_index(), max_index(), std::numeric_limits<record_index_t>::max());
}

std::unique_ptr<recordset> base_table_impl::get_recordset(record_index_t first, record_index_t last, record_index_t count) const
{
    lock_t lock{_mutex};
    ...
    {
        data.push_back( raw_record{*get_record(i)} );
    }
